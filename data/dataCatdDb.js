        ...
        tx.executeSql(
            ...
            (_, { rows: { _array } }) => {
                if (_array.length === 0) {
                    ...
                    tx.executeSql(
                        `INSERT INTO datacard (name, icon, color) VALUES (?, ?, ?)`,
                        ['Temperature', '../../assets/experienceIcons/temperature.png', '#FD9093']
                    );
                    tx.executeSql(
                        `INSERT INTO datacard (name, icon, color) VALUES (?, ?, ?)`,
                    ...
                    tx.executeSql(
                        `INSERT INTO datacard (name, icon, color) VALUES (?, ?, ?)`,
                        ['Orientation', '../../assets/experienceIcons/orientation.png', '#C2CAFC']
                    );
                    tx.executeSql(
                        `INSERT INTO datacard (name, icon, color) VALUES (?, ?, ?)`,
                    ...
                    tx.executeSql(
                        `INSERT INTO datacard (name, icon, color) VALUES (?, ?, ?)`,
                        ['Brightness', '../../assets/experienceIcons/luminosite.png', '#C2FEFE']
                    );
                    tx.executeSql(
                        `INSERT INTO datacard (name, icon, color) VALUES (?, ?, ?)`,
            ...
            (_, { rows: { _array } }) => {
                if (_array.length === 0) {
                    tx.executeSql(
                        ...
                        ['Trus', "Confiance: La confiance est le sentiment de fiabilité et de crédibilité envers quelqu''un ou quelque chose.", 'Trust', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Comfort', "Confort: Le confort représente le sentiment de bien-être et de détente physique ou psychologique.", 'Comfort', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Fear', "Peur: La peur est une émotion ressentie face à une menace, un danger ou quelque chose d''inconnu.", 'Fear', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Control', "Contrôle: Le contrôle implique le pouvoir de diriger, réguler ou influencer une situation ou un événement.", 'Control', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Effi\nciency', "Efficacité: L''efficacité est la capacité à atteindre un objectif ou produire un résultat avec le minimum de ressources utilisées.", 'Efficiency', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Learn\nability', "Facilité d''apprentissage: La facilité d''apprentissage désigne la facilité avec laquelle on peut acquérir de nouvelles compétences ou connaissances.", 'Learnability', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Ease of use of use', "Facilité d''utilisation: La facilité d''utilisation indique la simplicité et la praticité d''utilisation d''un produit ou d''un système.", 'Ease of use', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Stress', "Stress: Le stress est une réponse physique ou émotionnelle à une pression ou à une situation difficile.", 'Stress', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Intuiti\nvity', "Intuitivité: L''intuitivité se rapporte à la facilité de compréhension ou d''utilisation d''une chose sans besoin d''explications complexes.", 'Intuitivity', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Perfor\nmance', "Performance: La performance représente la qualité d''exécution ou de fonctionnement d''une personne, d''un produit ou d''un système.", 'Performance', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Relia\nbility', "Fiabilité: La fiabilité est la capacité d''une personne ou d''un objet à maintenir des standards élevés de qualité et de performance de manière consistante.", 'Reliability', '#FFF']
                    );
                    tx.executeSql(
                        `INSERT INTO experience (name, definition, experience, color) VALUES (?, ?, ?, ?)`,
                        ['Safety', "Sécurité: La sécurité est l''état de protection contre les dangers, les risques ou les menaces pour éviter les dommages ou les blessures.", 'Safety', '#FFF']
                    );
                }else {
            ...
            (_, { rows: { _array } }) => {
                if (_array.length === 0) {
                    tx.executeSql(
                        ...
                        ['John Doe', 'ENSAM', 'Developer', 'john.doe@admin.com', 'admin', null]
                    );
    ...
    return new Promise((resolve, reject) => {
        db.transaction(tx => {
            tx.executeSql(
                ...
                [],
                (_, { rows: { _array } }) => {
                    const dataCards = _array.map(({ id, name, icon, color }) => ({ id, name, icon, color }));
                    resolve(dataCards);
                },
                (_, error) => {
                    reject(error);
    ...
    return new Promise((resolve, reject) => {
        db.transaction(tx => {
            tx.executeSql(
                ...
                [],
                (_, { rows: { _array } }) => {
                    const experiences = _array.map(({ id, name, definition, experience, color }) => ({ id, name, definition, experience, color }));
                    resolve(experiences);
                },
                (_, error) => {
                    reject(error);
